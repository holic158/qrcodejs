 ...
 * @see <a href="http://jeromeetienne.github.com/jquery-qrcode/" target="_blank">http://jeromeetienne.github.com/jquery-qrcode/</a>11
 */
var QRCode;
...
// 즉시 실행함수 왜 쓰나?
// => 한 번의 실행만 필요로 하는 초기화 코드 부분에 사용
// => 전역에 변수를 추가하지 않아도 되서 코드 충돌 없이 구현 가능
...
(function() {
    ...
    function QR8bitByte(data) {
        ...
        // function 안에서 this?
        // 생성자 용도 (this.data)
        // QR8bitByte의 mode, data, parsedData 객체를 만들어서 사용함.

        // CHECK
        // this.mode = 4;
        // 4가 나오는데 왜 8bit_byte로 정의했지?
        this.mode = QRMode.MODE_8BIT_BYTE;
        this.data = data;
        this.parsedData = [];

        // Added to support UTF-8 Characters
        // for 문에 조건을 두 개 걸 수 있다.
        // i = 0 이고, l은 데이터 길이 일 때, i가 l보다 작고 i ++;
        for (var i = 0, l = this.data.length; i < l; i++) {
            var byteArray = [];
            // charCodeAt() 메서드는 index에 해당하는 문자의 unicode 값을 리턴함
            // http://www.unicode.org/charts/PDF/UAC00.pdf 에서
            // 한글 아무거나 ex '자'를 찾아보면 'C790'인데 이것은 16진수다. 
            ...
            // 뭔지 모르겠지만 data를 코드로 바꿔서 parsedData에 집어 넣음.
            this.parsedData.push(byteArray);
        }


        // https://www.bsidesoft.com/323
        // prototype : 브라우저는 배열에 concat 함수 내장하고 있음
        // 다만 Array와 관련된 Native 함수를 사용하면 성능이 더 조음 개 조음
        // TO-BE
        // Array 브라우저 함수 vs Native 함수 성능 비교 해보기.
    ...
    // QR8bitByte.prototype이라는 빈 Object에 함수 저장함.
    // 몇 번 QR8bitByte 객체를 만들 던 관계없이 한번 정의하고 저장함.
    ...
    // QRCodeModel.prototype 함수를 Object에 저장함.
    // 몇 번 QRCodeModel 객체를 만들던 관계 없이 한 번 정의 후 사용함.
    QRCodeModel.prototype = {
        // addData
        addData: function(data) {
            // QR8bitByte 객체 생성 후 netData에 삽입
            var newData = new QR8bitByte(data);

            console.log(JSON.stringify(newData)); // {"mode":4,"data":"https://google.com","parsedData":[104,116,116,112,115,58,47,47,103,111,111,103,108,101,46,99,111,109]}

            // dataList array에 newData push함.
            this.dataList.push(newData);
            this.dataCache = null;
        },
        // isDark
        // javascript throw new Error
        // error를 console에 발생시킴
        // 디버깅 시 굳이 console.log 나 alert 사용할 필요 없을 듯?
    ...
    // 오른쪽에 0을 밀어넣어 왼쪽으로 이동하고 가장, 왼쪽의 비트가 떨어져 나감(2진수)

    // MODE_NUMBER
    // 1 << 0
    // 0000 0001 : 1
    // 1을 0 비트(0회) 왼쪽으로 이동 하라
